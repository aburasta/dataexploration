# Third Reich Documentary — Character Angle Pool

The 9 AM daily task picks **one random `pending` row** each morning (see the runbook for the exact
selection rule), scripts it, and marks it `scripted`. This replaced the old sequential
`episode-queue.md` on 2026-07-08 — that file is left on disk untouched but is no longer read by
the automation. Add/remove/edit rows anytime; the full menu of underlying material is in
`chapter-by-chapter-hidden-gems-report.md` and the raw text is `source-text-full-book.txt`.

**Status flow:** `pending` → `scripted` (rough script written) → `final` (you saved
script-final.md) → `produced` (mp4 rendered). A row can also bounce back to `pending` if the
scripter starts it and finds the material too thin (see runbook Step 3 fallback rule) — in that
case leave a one-line note in the Angle title cell noting it was attempted and skipped once.

**Types** — each drives a different research method and structure (full detail in the runbook):
- **FACET** — one life-dimension of one person
- **ARC** — one person's/pair's full multi-chapter throughline
- **RELATIONSHIP** — two people, alternating POV between them
- **INSTITUTION** — formation of a mechanism/body, story-first cold open, no single protagonist
- **EVENT** — a single dramatic incident, chapter-anchored (same shape as the old queue)

| # | Type | Figure(s) | Angle title | Core hook | Primary ch. | Status |
|---|------|-----------|-------------|-----------|--------------|--------|
| 1 | FACET | Hitler | The Making of a Loner: Hitler's Vienna Years | Homeless, art-school rejection, no relations with women — the isolation that hardened into ideology | 1, 4 | pending |
| 2 | FACET | Hitler | The Education of Adolf Hitler | Poetsch the teacher, self-directed reading, Mein Kampf's stated intellectual debts | 1, 4 | pending |
| 3 | FACET | Hitler | The Complete Love Life of Adolf Hitler | Geli Raubal, Eva Braun, the minor infatuations, the bunker marriage | 5, 6, 27, 31 | produced |
| 4 | FACET | Hitler | Hitler the Soldier | The Iron Cross, the "white crow" who never wrote home, the war that forged his worldview | 1 | pending |
| 5 | FACET | Hitler | Hitler and Money | Unemployed drifter to villa owner — the income tax files that surfaced after the war | 5 | pending |
| 6 | FACET | Hitler | Hitler's War on the Churches | The plan to paganize Christianity, Niemöller's arc from supporter to prisoner | 8 | produced |
| 7 | FACET | Hitler | Hitler and His Generals | Twelve years of contempt, purges, and the slow death of institutional pushback | 10, 23, 26 | pending |
| 8 | FACET | Hitler | The Ideas Hitler Stole | Nietzsche, Wagner, and how borrowed philosophy became state ideology | 4 | pending |
| 9 | FACET | Hitler | Hitler in Prison | The Beer Hall Putsch trial as propaganda victory, writing Mein Kampf behind bars | 2, 3 | pending |
| 10 | FACET | Hitler | Hitler's Rise Through the Party | Anonymous agitator to Führer, 1919–1923 — the internal takeover | 2 | pending |
| 11 | FACET | Hitler | Hitler's Table Talk | What he said at headquarters when he thought he was just reminiscing, not making history | 23–26 | pending |
| 12 | FACET | Hitler | Hitler and Mussolini | Junior partner turned puppet-master, and what the phone call at Munich reveals | 12, 28 | produced |
| 13 | FACET | Hitler | Hitler's Last Year | Delusion, phantom armies, and a worldview collapsing faster than the front lines | 30, 31 | pending |
| 14 | FACET | Goebbels | Goebbels' Diaries: A Neurotic Love Life | Anke, Else, Alma — the private romantic chaos behind the propaganda genius | 5 | pending |
| 15 | FACET | Goebbels | The Goebbels Family Murder-Suicide | The only senior leader whose apocalyptic ideology extended to his own six children | 31 | pending |
| 16 | FACET | Himmler | Himmler's Posen Speech | The private confession of the Holocaust's architect, in his own unguarded words | 27 | pending |
| 17 | FACET | Göring | Göring: Flying Ace to Corrupt Reichsmarschall | WWI hero to the man who boasted "call me Meier," then lost an empire | 22 | pending |
| 18 | FACET | Speer | Speer's Secret Sabotage | The plot to gas Hitler's bunker, then flying into besieged Berlin to confess disobedience | 31 | pending |
| 19 | FACET | Ribbentrop | Ribbentrop's Reversals | The champagne salesman who assured Poland, then demanded Danzig a month after Munich | 14 | pending |
| 20 | ARC | Himmler | Himmler: Chicken Farmer to Genocide Architect | Ch.5 obscurity → Ch.27 machinery → Ch.30–31 betrayal, full throughline | 5, 27, 30, 31 | pending |
| 21 | ARC | Otto Skorzeny | The Commando Hitler Kept Sending Back Out | Gran Sasso → Operation Greif → the Horthy kidnapping → a comfortable postwar life | 28, 30 | pending |
| 22 | ARC | Franz Halder | The General Who Almost Arrested Hitler Twice | The 1938 Munich conspiracy, then dismissed in 1942 for telling the truth about Soviet strength | 12, 19, 26 | pending |
| 23 | ARC | Kurt von Schleicher | The Schemer Devoured by His Own Creation | Engineered his way to Chancellor, then murdered by the man he thought he controlled | 6, 7 | pending |
| 24 | ARC | Schacht & Neurath | The Technocrats Swept Aside in a Single Day | Feb 4, 1938 — the day every non-fanatic lost their post | 9, 10 | pending |
| 25 | ARC | Paul Schmidt | The Interpreter Who Heard Everything | Sole witness in every major room — Berchtesgaden, Godesberg, the Hácha coercion — while secretly signaling the resistance | 11–13 | pending |
| 26 | ARC | Galeazzo Ciano | Mussolini's Son-in-Law and the Diaries That Survived | Executed at Hitler's insistence; his wife smuggled the diaries out under her clothing | 28 | pending |
| 27 | ARC | Reinhard Heydrich | The Frame-Up Artist | The Fritsch fabrication, the Kristallnacht teletypes, the Wannsee paper trail | 10, 13, 27 | pending |
| 28 | ARC | Martin Bormann | The Secretary Who Outlasted Everyone | The quiet operator who out-maneuvered Göring, Himmler, and the entire bunker | 30, 31 | pending |
| 29 | RELATIONSHIP | Hitler & Röhm | Comrades to Corpses | Fourteen years of partnership ended with a pistol left on a table | 7 | pending |
| 30 | RELATIONSHIP | Hitler & Rommel | The Popular General He Couldn't Publicly Destroy | Why the Desert Fox got a forced suicide and a state funeral instead of a trial | 26, 29 | pending |
| 31 | RELATIONSHIP | Himmler & Fegelein | Family, Patronage, Betrayal | A former jockey marries into the Braun family, then dies for his patron's treason | 30, 31 | pending |
| 32 | RELATIONSHIP | Goebbels & the Strassers | The Battle for the Nazi Party's Soul | Hanover vs. Bamberg — the internal-socialism faction Hitler had to crush | 5 | pending |
| 33 | RELATIONSHIP | Chamberlain & Hitler | The Diplomacy of Self-Deception | Four meetings, one Englishman convinced he understood a man who was lying every time | 11–13 | pending |
| 34 | RELATIONSHIP | Stalin & his warners | Everyone Who Tried to Tell Him | Sam Woods, Schulenburg, even TASS's own denial — and the one man who believed none of it | 23 | pending |
| 35 | INSTITUTION | — | The Formation of the Nazi Party | Drexler's locksmith club to Hitler's machine — and the founder erased for building it | 2 | pending |
| 36 | INSTITUTION | — | The Formation of the SS | 200 men under an obscure chicken farmer to an empire that ran the Holocaust | 5, 27 | pending |
| 37 | INSTITUTION | — | The Formation of the Gestapo | How a state built an architecture of fear from scratch | 7, 8 | pending |
| 38 | INSTITUTION | — | The Birth of the "Stab in the Back" Myth | One British general's offhand question became the republic's founding lie | 2 | pending |
| 39 | INSTITUTION | — | The Legal Machinery of Dictatorship | The Feb 28, 1933 decree and the Enabling Act — the paperwork the whole Reich stood on | 1, 7 | pending |
| 40 | INSTITUTION | — | Building the Führer Myth | The propaganda techniques — staged theater, plebiscites, dual-consciousness deception — that built a god | 7, 8 | pending |
| 41 | INSTITUTION | — | The Formation of the July 20 Resistance | How an aristocratic/military conspiracy actually came together, and why it stayed isolated | 29 | pending |
| 42 | INSTITUTION | — | The Bureaucracy of the Holocaust | From subordinate memos to a killing machine — Wannsee's paper trail | 27 | pending |
| 43 | INSTITUTION | — | The Formation of the Nazi-Soviet Pact | Ideological enemies, a begging telegram, and a partition deal | 15 | pending |
| 44 | INSTITUTION | — | The Formation of the Axis | A sham alliance revealed by Mussolini's own backing-out letter | 16 | pending |
| 45 | EVENT | — | "Canned Goods": The Corpses That Started World War II | Naujocks, Gleiwitz, condemned prisoners in Polish uniforms | 15, 17 | scripted |
| 46 | EVENT | Hitler | Hitler's Most Inexplicable Decision | Declaring war on the US off one casual promise to Matsuoka, zero Pearl Harbor foreknowledge | 25 | pending |
| 47 | EVENT | Hácha | The 1 A.M. Meeting That Erased a Country | Threatened, fainted, revived by injection, signed away Czechoslovakia | 13 | scripted |
| 48 | EVENT | Drexler/Eckart | The Man Who Founded the Nazi Party and Was Erased for It | The libel suit that handed Hitler a party that wasn't his | 2 | pending |
| 49 | EVENT | — | Hitler Published the Whole Plan in 1925 | Mein Kampf's explicit Lebensraum blueprint, and the world's willful blindness to it | 4 | pending |
| 50 | EVENT | Stalin | Stalin Was Warned by Everyone and Believed No One | The intelligence, the TASS denial, the paranoia from the Hess flight | 23 | pending |
| 51 | EVENT | — | The Mud Defeated Germany Before the Winter Did | Rasputitsa vs. the "General Winter" myth, generals rereading Napoleon in real time | 24 | pending |
| 52 | EVENT | Paulus | The Field Marshal Promotion Designed as a Suicide Order | Promoted by radio so he'd shoot himself — he surrendered instead | 26 | pending |
| 53 | EVENT | Rommel | The Island Hitler Forgot | Malta postponed for Russia, and the direct mechanical link to El Alamein | 26 | produced |
| 54 | EVENT | Brautigam | The Nazi Official Who Told Hitler the Truth | An internal 1942 memo warning German brutality was uniting Russians against the Reich | 27 | pending |
| 55 | EVENT | — | Horoscopes in the Bunker | Goebbels reading Frederick the Great to Hitler, champagne when Roosevelt died, days before the end | 30, 31 | pending |

## Notes for the daily scripter
- Heavy-material angles (anything touching Ch. 27 directly, or the Goebbels family murder-suicide,
  #15) drop the catchy cold-open register for sober documentary narration — same sensitivity rule
  as every other series, and it's content-based, not chapter-number-based: if the core subject is
  atrocity/genocide mechanics or a family's death, sober mode applies regardless of which row it is.
- #6 and #11 are flagged lower-confidence on depth — if research turns up too little, apply the
  thin-material fallback in the runbook (swap to the next pending row, leave a note here, don't
  force a script).
- Backlog log (appended by the 9 AM task each morning): unscripted-and-unedited rough scripts
  awaiting your edit.
- Backlog 2026-07-08: 0 rough scripts awaiting edit (pool just created, nothing scripted yet).
- Backlog 2026-07-08: 1 rough script awaiting edit (ep003 — hitler-love-life).
- Backlog 2026-07-10: 0 rough scripts awaiting edit. ep003 (hitler-love-life) rendered end-to-end
  from the user's own bracket-tagged script-rough.md (media-placement markers, no separate
  script-final.md save) — see the media-assembly memory note for the new standard method.
- Backlog 2026-08-04: 2 new rough scripts written (row #45 Gleiwitz "Canned Goods", row #47 The Ordeal of Dr. Hácha), grounded in ch.15/17 and ch.13. Also corrected stale statuses: #6 war-on-religion and #12 Hitler & Mussolini are produced. Separately produced this period: special-reichstag-fire-1933 and special-hitler-name-schicklgruber (not original pool rows).
- Backlog 2026-08-04 (later): row #53 'The Island Hitler Forgot' scripted (cinematic, ~2.3k words) and produced end-to-end as a 16-min silent visual cut (special-malta-island-hitler-forgot-1942). Also produced this session: special-reichstag-fire-1933 (14.2-min silent cut). Mussolini episode media/spec built but render deferred at user request.
